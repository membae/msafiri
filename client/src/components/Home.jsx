...
function Home() {
  ...
  useEffect(() => {
    // Fetch buses only if from and to are selected
    if (formData.from && formData.to) {
      fetch("http://localhost:5000/buses")
        .then(res => res.json())
        .then(data => {
          // Filter buses that have available seats
  ...
  const handleBooking = async (e) => {
    ...
    try {
      const token = localStorage.getItem("access_token"); // assuming user is logged in
              ...
              {formData.bus_id && (
                <>
                  ...
                  <input
                    ...
                    min="1"
                    max={
                      buses.find(bus => bus.id === parseInt(formData.bus_id))?.capacity || 1
                    }
